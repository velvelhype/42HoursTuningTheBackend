...
const getLinkedUser = async (headers) => {
  ...
  mylog(target);
  const qs = `select 1 from session where value = ?`;
...
const postRecords = async (req, res) => {
  ...
  let [rows] = await pool.query(
    `select 1 from group_member where user_id = ?
    AND is_primary = true`,
    [user.user_id],
  );
...
const getRecord = async (req, res) => {
  ...
  const recordQs = `select 1 from record where record_id = ?`;
  ...
  const searchPrimaryGroupQs = `select 1 from group_member where user_id = ? and is_primary = true`;
  const searchUserQs = `select 1 from user where user_id = ?`;
  const searchGroupQs = `select 1 from group_info where group_id = ?`;
  const searchCategoryQs = `select 1 from category where category_id = ?`;
  ...
  const searchItemQs = `select 1 from record_item_file where linked_record_id = ? order by item_id asc`;
  const [itemResult] = await pool.query(searchItemQs, [line.record_id]);
  mylog('itemResult');
  mylog(itemResult);

  const searchFileQs = `select 1 from file where file_id = ?`;
...
const tomeActive = async (req, res) => {
  ...
  const searchMyGroupQs = `select 1 from group_member where user_id = ?`;
  const [myGroupResult] = await pool.query(searchMyGroupQs, [user.user_id]);
  mylog(myGroupResult);

  const targetCategoryAppGroupList = [];
  const searchTargetQs = `select 1 from category_group where group_id = ?`;
  ...
  let searchRecordQs =
    'select 1 from record where status = "open" and (category_id, application_group) in (';
  let recordCountQs =
    'select count(1) from record where status = "open" and (category_id, application_group) in (';
  const param = [];
  ...
  const searchGroupQs = 'select 1 from group_info where group_id = ?';
  const searchThumbQs =
    'select 1 from record_item_file where linked_record_id = ? order by item_id asc limit 1';
  const countQs = 'select count(1) from record_comment where linked_record_id = ?';
  const searchLastQs = 'select 1 from record_last_access where user_id = ? and record_id = ?';
  ...
  for (let i = 0; i < recordResult.length; i++) {
    ...
    if (countResult.length === 1) {
      commentCount = countResult[0]['count(1)'];
    }
  ...
  if (recordCountResult.length === 1) {
    count = recordCountResult[0]['count(1)'];
  }
...
const allActive = async (req, res) => {
  ...
  const searchRecordQs = `select 1 from record where status = "open" order by updated_at desc, record_id asc limit ? offset ?`;
  ...
  const recordCountQs = 'select count(1) from record where status = "open"';
...
const allClosed = async (req, res) => {
  ...
  const searchRecordQs = `select 1 from record where status = "closed" order by updated_at desc, record_id asc limit ? offset ?`;
  ...
  const recordCountQs = 'select count(1) from record where status = "closed"';
...
const mineActive = async (req, res) => {
  ...
  const searchRecordQs = `select 1 from record where created_by = ? and status = "open" order by updated_at desc, record_id asc limit ? offset ?`;
  ...
  const recordCountQs = 'select count(1) from record where created_by = ? and status = "open"';
...
const getComments = async (req, res) => {
  ...
  const commentQs = `select 1 from record_comment where linked_record_id = ? order by created_at desc`;
...
const getCategories = async (req, res) => {
  ...
  const [rows] = await pool.query(`select 1 from category`);
